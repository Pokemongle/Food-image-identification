function total_SURF_Features = Extract_SURF_Features(data)
%提取LBP特征
% [~,~,totalNum] = size(data);
[~,totalNum] = size(data);
total_SURF_Features = [];
for i = 1 : totalNum
    ...
    SURF_Features = extractLBPFeatures(image2,'CellSize',[16,16],'Upright',true);
    ...
    SURF_CellHists = reshape(SURF_Features,numBins,[]);
    % Normalize each LBP cell histogram using L1 norm.
    SURF_CellHists = bsxfun(@rdivide,SURF_CellHists,sum(SURF_CellHists));
    % Reshape the LBP features vector back to 1-by- N feature vector.
    SURF_Features = reshape(SURF_CellHists,1,[]);
    total_SURF_Features = [total_SURF_Features;SURF_Features];
end
end
